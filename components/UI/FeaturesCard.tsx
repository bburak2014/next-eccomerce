			...
			<div className="flex gap-5">
				{
					data.slice(2).map((item, index) => (
						<button style={{width:"189px"}} key={index} onClick={() => setFeature(item.value)} className={`px-5 py-3 text-start flex flex-col gap-custom-7 w-145 ${item.value === feature ? "bg-white shadow-[0_5px_10px_rgba(0,0,0,0.1)]" : "border border-gray-custom-2 border-0.5 "} `}>
